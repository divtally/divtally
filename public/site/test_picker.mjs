...
// =====================================================================
//  D-0019 / D-0022 — VARIANT-DEFINING mods on a registered unique. D-0022
//  UNLOCKS them: each is now a NORMAL tier-controlled row — required by
//  DEFAULT (it defines the item), so the all-required default reproduces the
//  D-0019 exact-variant query byte-for-byte; but the user may set one Nice-
//  to-have or Not-needed. buildRareQuery emits the defining option/exact/seed
//  filter (VALUE from the affix's own identity, never the picker's min/max)
//  when the mod is ticked, and OMITS it when the user deselects / not-needs it.
// =====================================================================
...
console.log("· D-0022 — defining OPTION mod DEFAULTS required + emitted; all-ticked == the D-0019 exact-variant query");
{
  eqQ(bpc.buildRareQuery(VDEF, VORIG), VORIG, "no picks -> all-ticked default reproduces the exact-variant ORIG (unchanged)");
  eqQ(bpc.buildRareQuery(VDEF, VORIG, bpc.rareDefaultPicks(VDEF)), VORIG, "explicit default picks reproduce ORIG");
  const d = bpc.rareDefaultPicks(VDEF);
  eq(d.affix[0].tier, "required", "the defining mod DEFAULTS to tier=required (it defines the item)");
  ok(d.affix[0].ticked === true, "…and is ticked by default");
...
console.log("· D-0022 — unticking a defining mod DROPS its filter (supersedes D-0019's forced lock)");
{
  const p = bpc.rareDefaultPicks(VDEF);
  p.affix[0].ticked = false;    // the user deselects the DEFINING mod — now HONOURED (D-0022)
  const q = bpc.buildRareQuery(VDEF, VORIG, p);
  ok(!filterFor(q, "explicit.stat_1190333629"), "a deselected defining mod is OMITTED from the query");
  ok(!!filterFor(q, "explicit.stat_life"), "the ordinary optional roll (still ticked) is untouched");
  eq(statIds(q), ["explicit.stat_life"], "only the still-ticked filter remains");
  // the optional roll still unticks independently; the defining mod (kept required) stays with its value
  const p2 = bpc.rareDefaultPicks(VDEF); p2.affix[1].ticked = false;
  const q2 = bpc.buildRareQuery(VDEF, VORIG, p2);
  eq(statIds(q2), ["explicit.stat_1190333629"], "unticking only the optional roll leaves the required defining filter");
  eq(filterFor(q2, "explicit.stat_1190333629").value, { option: 88 }, "…still carrying its identity option value");
}

console.log("· D-0022 — tiers: all-required == the prior locked query; not-needed DROPS it; nice -> count group");
{
  // all-required (the default) via the survey tier path == the prior single-AND exact-variant query
  const allreq = bpc.rareDefaultPicks(VDEF); allreq.affix[1].tier = "required";   // affix[0] already required
  const grouped = bpc.buildRareQuery(VDEF, VORIG, bpc.tierGroups(VDEF, allreq));
  eqQ(grouped, VORIG, "all-required tiers reproduce the prior exact-variant single-AND query (locked behaviour)");
  ok(grouped.stats.length === 1 && grouped.stats[0].type === "and", "exactly one AND group when everything is required");
  // not-needed the DEFINING mod -> it is excluded (the whole point of D-0022)
  const p = bpc.rareDefaultPicks(VDEF);
  p.affix[0].tier = "notneeded"; p.affix[1].tier = "notneeded";
  const q = bpc.buildRareQuery(VDEF, VORIG, bpc.tierGroups(VDEF, p));
  const ids = q.stats.reduce((a, g) => a.concat(g.filters.map(f => f.id)), []);
  ok(ids.indexOf("explicit.stat_1190333629") < 0, "a not-needed defining mod is DROPPED (no longer forced into the AND group)");
  ok(ids.indexOf("explicit.stat_life") < 0, "the not-needed optional roll is dropped too");
  // nice-to-have the defining mod -> it lands in the COUNT group, still carrying its option value
  const pn = bpc.rareDefaultPicks(VDEF); pn.affix[0].tier = "nice"; pn.affix[1].tier = "notneeded";
  const cG = bpc.buildRareQuery(VDEF, VORIG, bpc.tierGroups(VDEF, pn)).stats.find(g => g.type === "count");
  ok(!!cG && cG.filters.some(f => f.id === "explicit.stat_1190333629" && JSON.stringify(f.value) === JSON.stringify({ option: 88 })),
     "a nice-to-have defining mod emits its option filter in the count group");
}

console.log("· D-0022 — defining EXACT seed: min==max when emitted (identity-locked), dropped when not-needed");
{
  ...
  eqQ(bpc.buildRareQuery(VSEED, VSEED_ORIG), VSEED_ORIG, "all-ticked reproduces the exact-seed query (unchanged default)");
  const p = bpc.rareDefaultPicks(VSEED); p.affix[0].min = 5000; p.affix[0].max = 9000;   // user edits min/max
  eq(filterFor(bpc.buildRareQuery(VSEED, VSEED_ORIG, p), "explicit.pseudo_timeless_jewel_kaom").value,
     { min: 11711, max: 11711 }, "the VALUE ignores picker min/max edits (the seed is the item's identity)");
  const p2 = bpc.rareDefaultPicks(VSEED); p2.affix[0].ticked = false;   // deselect the seed entirely
  const q2 = bpc.buildRareQuery(VSEED, VSEED_ORIG, p2);
  ok(!filterFor(q2, "explicit.pseudo_timeless_jewel_kaom"), "deselecting the sole defining seed drops it (scope-only search)");
  eq(statIds(q2), [], "no stat filters remain once the sole defining seed is not-needed");
  const p3 = bpc.rareDefaultPicks(VSEED); p3.affix[0].tier = "notneeded";
  ok(!filterFor(bpc.buildRareQuery(VSEED, VSEED_ORIG, bpc.tierGroups(VSEED, p3)), "explicit.pseudo_timeless_jewel_kaom"),
